...
export function buildCsv(state: FarmState) {
  const rows = [
    ["FINANÇAS"],
    ["Tipo", "Descrição", "Categoria", "Cultura", "Valor", "Data"],
    ...state.transactions.map((item) => [item.kind === "expense" ? "Despesa" : "Receita", item.description, item.category, item.culture ?? "Geral", item.amount.toFixed(2).replace(".", ","), formatFullDate(item.date)]),
    [],
    ["INVENTÁRIO"],
...
export function buildPdfHtml(state: FarmState) {
  ...
  const rows = state.transactions.map((item) => `<tr><td>${item.kind === "expense" ? "Despesa" : "Receita"}</td><td>${item.description}</td><td>${item.culture ?? "Geral"}</td><td>${formatCurrency(item.amount)}</td><td>${formatFullDate(item.date)}</td></tr>`).join("");
  ...
  return `<html><head><meta name="viewport" content="width=device-width" /><style>body{font-family:Arial,sans-serif;color:#173324;padding:24px}h1{color:#2F6B45}h2{margin-top:28px;color:#2F6B45}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{border:1px solid #DCE8DE;padding:7px;text-align:left;font-size:11px}th{background:#EEF6EF}.summary{display:flex;gap:24px;background:#F6F8F2;padding:14px}.summary b{font-size:18px}</style></head><body><h1>Relatório — Sítio Gestão</h1><p>Gerado em ${formatFullDate(new Date().toISOString())}</p><div class="summary"><div>Receitas<br><b>${formatCurrency(revenue)}</b></div><div>Despesas<br><b>${formatCurrency(expenses)}</b></div><div>Saldo<br><b>${formatCurrency(revenue - expenses)}</b></div></div><h2>Finanças</h2><table><tr><th>Tipo</th><th>Descrição</th><th>Cultura</th><th>Valor</th><th>Data</th></tr>${rows || "<tr><td colspan='5'>Nenhum lançamento registrado.</td></tr>"}</table><h2>Inventário</h2><table><tr><th>Item</th><th>Categoria</th><th>Quantidade</th><th>Mínimo</th><th>Validade</th></tr>${stock || "<tr><td colspan='5'>Nenhum item cadastrado.</td></tr>"}</table></body></html>`;
}

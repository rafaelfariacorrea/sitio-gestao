...
import { formatCurrency, formatDate } from "@/lib/farm-data";
    ...
    <ScreenContainer className="px-5 pt-6" edges={["top", "left", "right"]}>
      <FlatList
        ...
        ListHeaderComponent={<View className="pb-8">
          ...
          <SectionTitle title="Cotações de referência" action="Ver safras" /><View className="mb-6 rounded-2xl border border-border bg-surface p-4">{quotes.map((quote) => <View key={quote.product} className="mb-3 flex-row items-center justify-between last:mb-0"><View className="flex-row items-center"><View className="h-2 w-2 rounded-full bg-success" /><Text className="ml-2 text-sm font-semibold text-foreground">{quote.product}</Text></View><Text className="font-bold text-foreground">{formatCurrency(quote.value)} / {quote.unit}</Text></View>)}{!quotes.length ? <Text className="text-sm text-muted">Atualizando fonte Conab…</Text> : null}<Text className="mt-3 text-xs leading-4 text-muted">Referência de atacado. O preço pago ao produtor depende de qualidade, volume, praça e negociação.</Text></View>
